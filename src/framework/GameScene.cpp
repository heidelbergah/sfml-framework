...
void GameScene::on_enter()
{
    ...
    m_particle_systems.add_particle_system("bp", std::move(ps1));
    m_particle_systems.add_particle_system("bullets", std::move(bullets));
    ...
    m_shaders.add_shader("bloom", std::move(bloom));
...
void GameScene::handle_event()
{
    ...
    sf::Vector2f world_pos = m_render_texture->mapPixelToCoords(sf::Vector2i(scaled_pos), m_camera->get_view());
    while(const std::optional event = m_window->pollEvent())
    {
        ...
        if (const auto* keyPressed = event->getIf<sf::Event::KeyPressed>())
        {
            ...
            {
                m_particle_systems.set_particle_system_position("bp", {256, 128});
                m_particle_systems.set_particle_system_position("bullets", {256, 128});
                m_widgets.get_widget<Button>("button")->set_position({256, 128});
            }
            if(sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Num2))
            {
                m_particle_systems.set_particle_system_position("bp", {900, 400});
                m_particle_systems.set_particle_system_position("bullets", {900, 400});
            ...
            {
                m_camera->shake(2);
                m_particle_systems.get_particle_system("bullets")->add_particles(1, sf::Color(255, 255, 255), Vector{30, sf::degrees(180)}, 3);
            }
        ...
        {
            m_particle_systems.get_particle_system("bp")->add_particles(20, sf::Color::White, Vector{2, sf::degrees(0)}, 180);
        }
        ...
        if(const auto* mousePressed = event->getIf<sf::Event::MouseButtonPressed>())
        {
            ...
            {
                m_camera->shake(50);
                m_particle_systems.get_particle_system("bp")->add_particles(30, sf::Color(203, 53, 61), Vector{4, sf::degrees(0)}, 180);
                m_particle_systems.get_particle_system("bp")->add_particles(30, sf::Color(249, 182, 78), Vector{3, sf::degrees(0)}, 180);
                m_particle_systems.get_particle_system("bp")->add_particles(30, sf::Color(237, 98, 64), Vector{5, sf::degrees(0)}, 180);
                m_particle_systems.get_particle_system("bp")->add_particles(20, sf::Color(86, 61, 67), Vector{1, sf::degrees(0)}, 180);
                m_particle_systems.get_particle_system("bp")->add_particles(20, sf::Color(106, 74, 87), Vector{2, sf::degrees(0)}, 180);
            }

            if(sf::Mouse::isButtonPressed(sf::Mouse::Button::Right))
            {
                m_particle_systems.get_particle_system("bp")->set_position(world_pos);
                m_camera->shake(150);
                ...
                for(int i = 0; i < num_particles; ++i)
                {
                    ...
                    m_particle_systems.get_particle_system("bp")->add_particles(1, colors[rand_color], Vector{speed, sf::degrees(0)}, 180);
                }
            }
...
void GameScene::update(sf::Time delta_time)
{
    ...
    m_particle_systems.update(delta_time);
    m_particle_systems.sync_with_view(m_camera->get_position());
    m_camera->update();
}
...
void GameScene::render()
{
    ...
    m_render_texture->clear(sf::Color(50, 50, 50));
    ...
    m_render_texture->draw(m_particle_systems, m_shaders.get_shader("bloom").get());
    
    m_render_texture->display();

    m_render_sprite->setTexture(m_render_texture->getTexture());
